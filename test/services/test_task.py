...
class TestTaskService(unittest.TestCase):
    ...
    def test_get_video_materials_uses_imagine_source(self):
        params = VideoParams(video_subject="waves", video_source="imagine")
        with patch.object(
            tm.imagine_source,
            "generate_videos",
            return_value=["/tmp/clip.mp4"],
        ) as generate:
            result = tm.get_video_materials("task-1", params, ["ocean"], 10.0)
        self.assertEqual(result, ["/tmp/clip.mp4"])
        generate.assert_called_once()
        self.assertEqual(generate.call_args.kwargs["task_id"], "task-1")
        self.assertEqual(generate.call_args.kwargs["search_terms"], ["ocean"])
